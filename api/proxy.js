...
async function _handle(req, res) {
  ...
  // Diagnostic: ?debug=1 returns what the proxy actually received/built instead
  // of calling Discogs, so we can see whether the token arrived and got applied.
  if (req.query.debug === "1") {
    ...
    return res.status(200).json({
      ...
      method: req.method,
      receivedUrlParam: u,
      receivedTokenParam: token ? ("present (len " + String(token).length + ")") : "MISSING",
      queryKeys: Object.keys(req.query),
      incomingAuthHeader: incomingAuth || null,
